...
class UrdfLink(object):
  ...
  def __init__(self, name ='link', urdf_inertial = UrdfInertial(), urdf_visual_shapes = [], urdf_collision_shapes=[]):
    self.link_name = name
    self.urdf_inertial = urdf_inertial
    self.urdf_visual_shapes = urdf_visual_shapes
    self.urdf_collision_shapes = urdf_collision_shapes


class UrdfVisual(object):

  def __init__(self,origin_rpy = [0,0,0],origin_xyz = [0,0,0], material_name="",
              geom_type = p.GEOM_BOX, geom_radius = 1, geom_extents = [1,1,1], geom_length = 10,
              geom_meshfilename = 'meshfile',geom_meshscale = [1,1,1],material_rgba = [1,0,0,1] ):
    self.origin_rpy = origin_rpy
    self.origin_xyz = origin_xyz
    self.geom_type = geom_type
    self.geom_radius = geom_radius
    self.geom_extents = geom_extents
    self.geom_length = geom_length
    self.geom_meshfilename = geom_meshfilename
    self.geom_meshscale = geom_meshscale
    self.material_rgba = material_rgba
    self.material_name = material_name


class UrdfCollision(object):

  def __init__(self,origin_rpy = [0,0,0],origin_xyz = [0,0,0],
              geom_type = p.GEOM_BOX, geom_radius = 1, geom_extents = [1,1,1], geom_length = 10,
              geom_meshfilename = 'meshfile',geom_meshscale = [1,1,1]):
    self.origin_rpy = origin_rpy
    self.origin_xyz = origin_xyz
    self.geom_type = geom_type
    self.geom_radius = geom_radius
